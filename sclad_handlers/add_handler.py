...
def handle_qr_cell_scan(bot, conn, message):
    if message.content_type == 'photo':
        ...
        if qr_data:
            ...
            # Передаем message как аргумент
            associate_equipment_to_cell(bot, conn, message.chat.id, cell_number, message)
        else:
...
def get_last_equipment_id(conn, message):
    cursor = conn.cursor()
...
def associate_equipment_to_cell(bot, conn, chat_id, cell_number, message):
    ...
    if cell:
        # Передаем conn и message в get_last_equipment_id
        equipment_id = get_last_equipment_id(conn, message)
